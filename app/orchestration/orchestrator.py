...
class Orchestrator:
    ...
    async def handle_request(self, request: ChatRequest, user_id: str = "anonymous") -> ChatResponse:
        ...
        if use_rag:
            # RAG Flow
            # Currently RAG pipeline returns just a string, we might need to refactor it 
            # to return usage stats too. For now, we wrap it.
            ...
            prompt_tokens = token_manager.estimate_tokens(request.message) # + context
